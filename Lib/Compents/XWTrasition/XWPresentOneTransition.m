...
@property (nonatomic, assign) XWPresentOneTransitionDirection direction;
...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    switch (_type) {
        case XWPresentOneTransitionTypePresent:
            [self presentAnimation:transitionContext direction:_direction];
            break;
            
        case XWPresentOneTransitionTypeDismiss:
            [self dismissAnimation:transitionContext direction:_direction];
            break;
    }
}
 ...
 *  实现present动画
 */
- (void)presentAnimation:(id<UIViewControllerContextTransitioning>)transitionContext direction:(XWPresentOneTransitionDirection)direction {
    ...
    toVC.view.frame = direction ? CGRectMake(-containerView.width, 0, containerView.width, containerView.height) : CGRectMake(containerView.width, 0, containerView.width, containerView.height);
    toVC.view.alpha = .5f;
    ...
    [UIView animateWithDuration:[self transitionDuration:transitionContext] delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
        //首先我们让vc2向上移动
        toVC.view.transform = CGAffineTransformMakeTranslation(direction ? containerView.width : -containerView.width , 0);
        toVC.view.alpha = 1;
        tempView.alpha = 0.7;
        //然后让截图视图缩小一点即可
...
//    [UIView animateWithDuration:[self transitionDuration:transitionContext] delay:0 usingSpringWithDamping:0.55 initialSpringVelocity:1.0 / 0.8 options:0 animations:^{
//        //首先我们让vc2向上移动
//        toVC.view.transform = CGAffineTransformMakeTranslation(direction ? containerView.width : -containerView.width , 0);
//        //然后让截图视图缩小一点即可
//        tempView.transform = CGAffineTransformMakeScale(0.85, 0.85);
 ...
 *  实现dimiss动画
 */
- (void)dismissAnimation:(id<UIViewControllerContextTransitioning>)transitionContext direction:(XWPresentOneTransitionDirection)direction {
